...
def main():
    ...
    match outy:
        case '1 pdb file':
            ...
            os.system(''.join(['curl -s https://files.rcsb.org/view/',args['pdb id'],'.pdb',' > ',args['pdb id'],'.pdb']))
        case '1 fasta file':
            os.chdir(args['output directory'])
            os.system(''.join(['curl -s https://www.rcsb.org/fasta/entry/',args['pdb id'],'/display',' > ',args['pdb id'],'.fasta']))
        case 'many pdb files':
            ids = (str(line.rstrip()) for line in open(args['input txt']))
            os.chdir(args['output directory'])
            for i in ids:
                os.system(''.join(['curl -s https://files.rcsb.org/view/',i,'.pdb',' > ',i,'.pdb']))
        case 'many fasta files':
            ids = (str(line.rstrip()) for line in open(args['input txt']))
            os.chdir(args['output directory'])
            for i in ids:
                os.system(''.join(['curl -s https://www.rcsb.org/fasta/entry/',i,'/display',' > ',i,'.fasta']))
        case '1 multi-fasta file':
            ids = (str(line.rstrip()) for line in open(args['input txt']))
            for i in ids:
                os.system(''.join(['curl -s https://www.rcsb.org/fasta/entry/',i,'/display',' >> ',args['multi fasta']]))        
